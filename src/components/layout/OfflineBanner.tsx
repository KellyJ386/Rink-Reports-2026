...
export function OfflineBanner() {
  ...
  return (
    <div className="bg-alert-yellow/10 text-alert-yellow-dark dark:text-alert-yellow border-b border-alert-yellow/20 px-4 py-2 flex items-center justify-center gap-2 text-sm">
      {!isOnline ? (
        <>
          <Icon name="wifi-off" size={16} />
          <span>You are offline. Changes will sync when connected.</span>
          {pendingSync > 0 && (
            <span className="ml-2 bg-navy dark:bg-gray-700 text-white text-xs px-2 py-0.5 rounded-full">
              {pendingSync} pending
            </span>
          )}
        </>
